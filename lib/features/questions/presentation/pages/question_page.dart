...
class QuestionPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<QuestionsCubit>()..getQuestions(examId),
      child: BlocConsumer<QuestionsCubit, BaseState<ExamData>>(
        ...
        listener: (context, state) {
          ...
          if (state.data?.result != null) {
            ...
            Navigator.pushReplacement(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    ExamScorePage(result: state.data!.result!),
              ),
            );
            return;
          }
                    ...
                    Column(
                      children: question.answers.map((answer) {
                        return AnswerWidget(
                          ...
                          onSelect: (value) {
                            context.read<QuestionsCubit>().selectAnswer(
                              question.id,
                              value,
                            );
                          },
                      ...
                      children: [
                        if (state.data!.currentIndex > 0)
                          QuestionButton(
                            ...
                            onPressed: () {
                              context.read<QuestionsCubit>().previousQuestion();
                            },
                          ),
                        QuestionButton(
